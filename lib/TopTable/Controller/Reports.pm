...
sub auto :Private {
  ...
  push( @{ $c->stash->{breadcrumbs} }, {
    ...
    label => $c->maketext("menu.text.reports"),
  });
}
...
sub base :Chained("/") :PathPart("reports") :CaptureArgs(1) {
  ...
  if ( grep( $_ eq $report_id, @reports ) ) {
    ...
    my $encoded_report_name = encode_entities( $c->maketext( sprintf( "reports.name.%s", $report_id ) ) );
    
    $c->stash({
      report_id           => $report_id,
      report_name         => $c->maketext( sprintf( "reports.name.%s", $report_id ) ),
      encoded_report_name => $encoded_report_name,
    });
...
sub view_specific_season :Chained("view") :PathPart("seasons") :Args(1) {
  ...
  if ( defined( $season ) ) {
    ...
    $c->stash({
      ...
      encoded_season_name => $encoded_season_name,
      page_description    => $c->maketext("description.reports.view-specific", $report_name, $encoded_season_name, $site_name),
    });
  } else {
    # Invalid season
...
sub view_finalise :Private {
  ...
  my $encoded_report_name = $c->stash->{encoded_report_name};
  ...
  $c->stash({
    ...
    external_scripts    => [
      ...
      $c->uri_for( sprintf( "/static/script/reports/%s.js", $report_id ) ),
    ...
    view_online_display => sprintf( "Viewing %s", $encoded_report_name ),
    view_online_link    => 1,
