...
while ( my $person = $people->next ) {
  ...
  while ( my $team_association = $team_associations->next ) {
    ...
    if ( $matches_played->count != $team_association->matches_played or $matches_won != $team_association->matches_won or $matches_drawn != $team_association->matches_drawn or $matches_lost != $team_association->matches_lost ) {
      ...
      if ( $matches_played->count != $team_association->matches_played ) {
        printf "Officially played: %d, updated to: %d\n", $team_association->matches_played, $matches_played->count;
        $team_association->matches_played( $matches_played->count );
      }
      
      if ( $matches_won != $team_association->matches_won ) {
        printf "Officially won: %d, updated to: %d\n", $team_association->matches_won, $matches_won;
        $team_association->matches_won( $matches_won );
      }
      
      if ( $matches_drawn != $team_association->matches_drawn ) {
        printf "Officially drawn: %d, updated to: %d\n", $team_association->matches_drawn, $matches_drawn;
        $team_association->matches_drawn( $matches_drawn );
      }
      
      if ( $matches_lost != $team_association->matches_lost ) {
        printf "Officially lost: %d, updated to: %d\n", $team_association->matches_lost, $matches_lost;
        $team_association->matches_lost( $matches_lost );
      }
